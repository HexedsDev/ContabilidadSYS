...
const DEFAULT_WORDS = ['CONTABILIDAD', 'PARTIDAS', 'REPORTES', 'RESPALDO', 'CONTROL'];
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className }: ParticleTextEffectProps) {
  ...
  useEffect(() => {
    ...
    const paintWord = (word: string) => {
      ...
      offscreenCtx.font = `bold ${Math.max(48, Math.min(140, Math.round(height * 0.22)))}px Inter, Arial, sans-serif`;
    ...
    const resizeCanvas = () => {
      ...
      paintWord(words[wordIndexRef.current] ?? words[0]);
    };
    ...
    const animate = () => {
      ...
      currentCtx.fillStyle = 'rgba(2, 6, 23, 0.13)';
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        particle.move();
        particle.draw(currentCtx, drawAsPoints);
      ...
      if (frameCountRef.current % 240 === 0 && words.length > 0) {
        wordIndexRef.current = (wordIndexRef.current + 1) % words.length;
        paintWord(words[wordIndexRef.current]);
      }
  ...
  return (
    <div
      ref={containerRef}
      className={cn('relative w-full overflow-hidden rounded-2xl border border-white/10 bg-black', className)}
    >
